...
@section('content')
    ...
    <div class="max-w-6xl">
        ...
        <div class="flex items-center gap-3 mb-6">
            <div>
                <h1 class="text-[18px] font-bold text-[#092C4C]">Edit Kelas</h1>
                <p class="text-[12px] text-slate-400">Perbarui informasi kelas dan anggota siswa</p>
            </div>
        </div>

        @if ($errors->any())
            <div class="mb-5 bg-red-50 border border-red-200 rounded-[12px] px-5 py-4">
                <p class="text-sm font-semibold text-red-600 mb-2">Terdapat kesalahan:</p>
        ...
        <form action="{{ route('admin.kelas.update', $kela->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-5 gap-5 mb-5">
                    ...
                    <div>
                        ...
                        <div class="logo-drop" id="logoDrop">
                            ...
                            <div id="logoPlaceholder"
                                class="flex flex-col items-center justify-center py-6 gap-2 pointer-events-none {{ $kela->logo ? 'hidden' : '' }}">
                            ...
                            <div id="logoPreviewWrap"
                                class="{{ $kela->logo ? 'flex' : 'hidden' }} items-center justify-center py-4">
                                <img id="logoPreview"
                                    src="{{ $kela->logo ? asset('storage/' . $kela->logo) : '' }}"
                                    class="w-20 h-20 rounded-full object-cover border-2 border-[#00A99D] shadow">
                            </div>
                        </div>
                        @if ($kela->logo)
                            <p class="text-[11px] text-slate-400 mt-1.5">Logo saat ini tersimpan. Upload baru untuk menggantinya.</p>
                        @endif
                    </div>

                    {{-- Nama Kelas --}}
                    <div>
                        <label class="block text-[13px] font-semibold text-[#092C4C] mb-2">
                    ...
                    <div>
                        ...
                        <select name="subject_id" class="form-input" required>
                            ...
                            @foreach ($subjects as $s)
                                <option value="{{ $s->id }}" @selected(old('subject_id', $kela->subject_id) == $s->id)>
                                    {{ $s->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="teacher_id" class="form-input" required>
                            ...
                            @foreach ($teachers as $t)
                                <option value="{{ $t->id }}" @selected(old('teacher_id', $kela->teacher_id) == $t->id)>
                                    {{ $t->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="lg:col-span-2 bg-white rounded-[18px] border border-slate-200 shadow-sm p-7 flex flex-col">
                    <div class="flex-1 flex flex-col">
                        ...
                        <textarea name="description" id="descInput" maxlength="210"
                            class="form-input flex-1" style="min-height: 120px;"
                            placeholder="Tuliskan deskripsi singkat tentang kelas ini...">{{ old('description', $kela->description) }}</textarea>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="overflow-hidden rounded-[15px] border border-slate-200 flex flex-col">
                        ...
                        <div class="bg-white border-t border-slate-200 px-5 py-4 flex items-center justify-between">
                            <button type="button" onclick="prev('all')" id="prevAll"
                                class="text-[13px] font-medium text-slate-600 hover:text-[#0F4C5C] px-3 py-1.5 border border-slate-200 hover:bg-slate-50 rounded-lg transition">← Prev</button>
                            <span id="pageAll" class="text-[13px] text-slate-500 font-medium"></span>
                            <button type="button" onclick="next('all')" id="nextAll"
                                class="text-[13px] font-medium text-slate-600 hover:text-[#0F4C5C] px-3 py-1.5 border border-slate-200 hover:bg-slate-50 rounded-lg transition">Next →</button>
                        </div>
                    </div>
                    ...
                    <div class="overflow-hidden rounded-[15px] border border-slate-200 flex flex-col shadow-sm">
                        ...
                        <div class="bg-white border-t border-slate-200 px-5 py-4 flex items-center justify-between">
                            <button type="button" onclick="prev('selected')" id="prevSelected"
                                class="text-[13px] font-medium text-slate-600 hover:text-[#0F4C5C] px-3 py-1.5 border border-slate-200 hover:bg-slate-50 rounded-lg transition">← Prev</button>
                            <span id="pageSelected" class="text-[13px] text-slate-500 font-medium"></span>
                            <button type="button" onclick="next('selected')" id="nextSelected"
                                class="text-[13px] font-medium text-slate-600 hover:text-[#0F4C5C] px-3 py-1.5 border border-slate-200 hover:bg-slate-50 rounded-lg transition">Next →</button>
                        </div>
                    </div>
            ...
            <div class="flex justify-end gap-3 mt-10">
                <a href="{{ route('admin.kelas.index') }}"
                    class="inline-flex items-center gap-2 px-5 py-2.5 rounded-[10px] border border-slate-200 text-[13px] text-slate-500 hover:bg-slate-50 transition">
                    Batalkan
                </a>
                <button type="submit"
                    class="inline-flex items-center gap-2 bg-[#00A99D] hover:bg-[#008f87] text-white px-7 py-2.5 rounded-[10px] text-[13px] font-semibold shadow-md shadow-teal-500/20 transition">
                    Simpan Perubahan
                </button>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            descInput.addEventListener('input', updateChar);
            updateChar();

            /* ===== Student Picker ===== */
            const students = @json($students);
            let selected   = @json($selectedStudents);
            ...
            function renderAll() {
                ...
                document.getElementById('allTable').innerHTML = rows.length
                    ? rows.map(s => studentRow(s, selected.includes(s.id), students.findIndex(x => x.id === s.id))).join('')
            ...
            function renderSelected() {
                ...
                document.getElementById('selectedTable').innerHTML = rows.length
                    ? rows.map(s => studentRow(s, true, students.findIndex(x => x.id === s.id))).join('')
            ...
            function render() {
                renderAll();
                renderSelected();
                syncHidden(); // ← selalu sync setiap state berubah
            }
