        ...
        <div
            ...
            <div class="flex items-center gap-4 w-full md:w-auto border-b md:border-b-0 border-slate-100 pb-4 md:pb-0">
                <div class="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 overflow-hidden flex-shrink-0">
                    <img src="{{ optional($kelas->teacher)->profile_photo
                        ? asset('storage/' . $kelas->teacher->profile_photo)
                        : 'https://ui-avatars.com/api/?name=' .
                            urlencode(optional($kelas->teacher)->name ?? 'Teacher') .
                            '&background=044153&color=ffffff' }}"
                ...
                <div>
                    ...
                    <p class="text-[#092C4C] font-bold text-base leading-none">
                        {{ $kelas->teacher->name ?? 'Tidak diketahui' }}
                    </p>
                </div>
            </div>
                                ...
                                @if (auth()->id() === $item->user_id)
                                    <div class="relative" onclick="event.stopPropagation()">
                                        ...
                                        <div id="menu-pengumuman-{{ $item->id }}"
                                            ...
                                            <button type="button" onclick="openEditAnnouncement({{ $item->id }})"
                                                class="block w-full text-left px-4 py-2.5 text-xs font-medium text-slate-600 hover:bg-slate-50 hover:text-[#00A79D]">Edit</button>
                                            <form action="{{ route('student.announcements.destroy', $item->id) }}"
                    ...
                    @if ($upcomingTasks->count() > 0)
                        <div class="space-y-4">
                            @foreach ($upcomingTasks as $task)
                                <div class="group">
                                    <a href="{{ route('student.meetings.show', $task->id) }}" class="block">
                                        <p
                                ...
                                <div id="accordion-{{ $meeting->id }}"
                                    ...
                                    <div class="p-5 md:p-6 text-sm text-slate-600 border-t border-slate-100">
                                        ...
                                        <div class="mt-4 pt-4 border-t border-slate-200"><a
                                                href="{{ route('student.meetings.show', $meeting->id) }}"
    ...
    <div id="modalPengumuman" class="hidden fixed inset-0 z-[60] overflow-y-auto">
        ...
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div
                ...
                <form action="{{ route('student.announcements.store') }}" method="POST" enctype="multipart/form-data"
                    id="announcementForm">
                    @csrf
