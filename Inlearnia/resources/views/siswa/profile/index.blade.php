...
@section('content')
        ...
        <div class="flex-1 flex flex-col gap-5">
            ...
            <div class="bg-white rounded-[20px] shadow-sm border border-slate-200 p-7">
                <div class="flex items-center gap-2 mb-6 pb-4 border-b border-slate-100">
                    ...
                    <h3 class="text-[#092C4C] text-[16px] font-semibold">Informasi Siswa</h3>
                </div>

                <div class="flex items-center gap-5 mb-6">
                    <div class="w-14 h-14 rounded-full overflow-hidden border-2 border-blue-500 flex-shrink-0">
                        @if ($user->profile_photo)
                            <img src="{{ asset('storage/' . $user->profile_photo) }}" class="w-full h-full object-cover">
                        @else
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=3b82f6&color=ffffff&size=128" class="w-full h-full object-cover">
                        @endif
                    </div>
                    <div>
                        <div class="text-[#092C4C] font-bold text-[15px]">{{ $user->name }}</div>
                        <span class="bg-blue-100 text-blue-600 text-[11px] font-semibold px-3 py-0.5 rounded-full">Siswa</span>
                    </div>
                </div>
                ...
                <div class="flex justify-end mt-6">
                    <button type="button"
                        ...
                        class="bg-[#00A99D] hover:bg-[#008f84] text-white px-6 py-2.5 rounded-[10px] text-[13px] font-medium transition shadow-md shadow-blue-500/20 flex items-center gap-2">
    ...
    <x-ui.modal-form id="modalEditSiswa" title="Edit Profil Siswa" description="Perbarui data akun Anda"
        iconBg="#FFAE00" buttonColor="bg-[#FFAE00]">
        ...
        <form id="editProfileFormSiswa" action="{{ route('student.profile.update') }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PATCH')
            ...
            <div class="mb-5">
                ...
                <div id="editPhotoPreviewWrapper" class="mb-3 hidden">
                    ...
                    <img id="editPhotoPreview" src="" alt="Foto profil" class="w-14 h-14 rounded-full object-cover border border-slate-200">
                </div>
                <div class="flex items-center border border-slate-300 rounded-[10px] overflow-hidden h-[40px]">
                    <label for="editProfilePhoto" class="px-4 h-full flex items-center bg-slate-50 text-[13px] text-slate-600 font-medium border-r border-slate-300 cursor-pointer hover:bg-slate-100 transition whitespace-nowrap">
                        Pilih File
                    </label>
                    <span id="fileLabelEditProfile" class="px-4 text-[13px] text-slate-400 truncate">Tidak ada file dipilih</span>
            ...
            <div class="grid grid-cols-2 gap-4 mb-5">
                <div>
                    <label class="text-[13px] font-medium text-slate-600 block mb-2">Nama Lengkap <span class="text-red-500">*</span></label>
                    <input type="text" name="name" id="editName" class="w-full border border-slate-300 rounded-[10px] px-4 py-2 text-[13px]" placeholder="Nama lengkap" required>
                </div>
                <div>
                    <label class="text-[13px] font-medium text-slate-600 block mb-2">Email <span class="text-red-500">*</span></label>
                    <input type="email" name="email" id="editEmail" class="w-full border border-slate-300 rounded-[10px] px-4 py-2 text-[13px]" placeholder="Email" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mb-5">
                <div>
                    <label class="text-[13px] font-medium text-slate-600 block mb-2">Password Baru <span class="text-slate-400 font-normal">(opsional)</span></label>
                    <input type="password" name="password" class="w-full border border-slate-300 rounded-[10px] px-4 py-2 text-[13px]" placeholder="Kosongkan jika tidak diubah">
                </div>
                <div>
                    <label class="text-[13px] font-medium text-slate-600 block mb-2">Konfirmasi Password</label>
                    <input type="password" name="password_confirmation" class="w-full border border-slate-300 rounded-[10px] px-4 py-2 text-[13px]" placeholder="Kosongkan jika tidak diubah">
                </div>
            </div>
            ...
            <div class="mb-7">
                ...
                <input type="text" name="phone" id="editPhone" class="w-full border border-slate-300 rounded-[10px] px-4 py-2 text-[13px]" placeholder="No. telepon">
            </div>

            <div class="flex justify-end gap-3">
                <button type="button" onclick="toggleModal('modalEditSiswa')" class="px-5 py-2 rounded-[10px] border border-slate-300 text-[13px] text-slate-500 hover:bg-slate-50 transition">
                    Batalkan
                </button>
                <button type="submit" class="px-5 py-2 bg-[#FFAE00] hover:bg-[#e69c00] text-white text-[13px] rounded-[10px] transition">
                    Simpan Perubahan
                </button>
            </div>
        </form>
    ...
    <script>
        @if (session('success'))
            Swal.mixin({ toast: true, position: 'top-end', showConfirmButton: false, timer: 3000, timerProgressBar: true }).fire({ icon: 'success', title: '{{ session('success') }}' });
        @endif
