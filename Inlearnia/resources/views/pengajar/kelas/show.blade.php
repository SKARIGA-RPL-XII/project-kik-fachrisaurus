...
@section('content')
    ...
    <style>
        ...
        #announcement-editor-container .ql-editor,
        #edit-announcement-editor-container .ql-editor {
            min-height: 120px !important;
        }

        /* Card Content Auto-Height & Reset Gap */
        .ql-snow .ql-editor.reset-quill-margins {
            min-height: auto !important;
            padding: 0 !important;
        }

        .reset-quill-margins p {
            margin-bottom: 0.5em !important;
            padding: 0 !important;
        }

        .reset-quill-margins p:first-child {
            margin-top: 0 !important;
        }

        .reset-quill-margins p:last-child {
            ...
            <div class="absolute bottom-0 left-0 w-full p-6 md:p-8 z-20 flex flex-col justify-end h-full">
                <div class="flex flex-col gap-1 transition-transform duration-500 group-hover:-translate-y-2">
                    <div class="flex items-center gap-3">
                        ...
                        <span
                            class="bg-[#00A79D] text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow-md">
                    ...
                    @if ($kelas->description)
                        <p
                            class="text-slate-200 text-xs md:text-sm mt-2 line-clamp-2 max-w-3xl drop-shadow-sm font-light leading-relaxed group-hover:text-slate-100 transition-colors">
                            {{ $kelas->description }}
                        </p>
                    @endif
                </div>
        ...
        <div
            ...
            <div class="flex items-center gap-4 w-full md:w-auto border-b md:border-b-0 border-slate-100 pb-4 md:pb-0">
                <div class="w-12 h-12 rounded-full bg-slate-100 border border-slate-200 overflow-hidden flex-shrink-0">
                    <img src="{{ auth()->user()->profile_photo ? asset('storage/' . auth()->user()->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=044153&color=ffffff' }}"
                ...
                <div>
                    ...
                    <p class="text-[#092C4C] font-bold text-base leading-none">
                        {{ $kelas->teacher->name ?? auth()->user()->name }}</p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-8 md:gap-12 w-full md:w-auto justify-around md:justify-end">
                <div class="text-center group/stat cursor-default">
                    <span
                        class="block text-xl font-bold text-orange-500 transition-colors">{{ $kelas->students_count }}</span>
                ...
                <div class="text-center group/stat cursor-default">
                    <span
                        class="block text-xl font-bold text-[#00A79D] transition-colors">{{ $kelas->meetings_count }}</span>
                    <span class="text-[11px] text-slate-400 font-bold uppercase tracking-wide">Pertemuan</span>
                </div>
                <div class="w-px h-8 bg-slate-200 hidden md:block"></div>
                <div class="text-center group/stat cursor-default">
                    <span
                        class="block text-xl font-bold text-purple-600 transition-colors">{{ $kelas->announcements_count }}</span>
    ...
    <div id="tab-content-beranda" class="tab-content block animate-fade-in">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-[15px] p-4 shadow-sm border border-slate-200 flex gap-4 items-center cursor-pointer hover:border-[#00A79D]/50 transition-all"
                    onclick="toggleModal('modalPengumuman')">
                    <div
                        class="w-10 h-10 rounded-full flex-shrink-0 bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200">
                        <img src="{{ auth()->user()->profile_photo ? asset('storage/' . auth()->user()->profile_photo) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=044153&color=ffffff' }}"
                            class="w-full h-full object-cover">
                    </div>
                    <div class="flex-1">
                        <div
                            class="flex items-center w-full px-4 py-2 rounded-full bg-slate-50 text-slate-500 text-sm font-medium border border-slate-200 hover:bg-slate-100 hover:border-slate-300 transition-all">
                    ...
                    <div class="w-9 h-9 rounded-full flex items-center justify-center" style="background-color: #CFF6F5;">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#00A79D"
                ...
                @forelse($feeds as $item)
                        ...
                        <div class="bg-white rounded-[15px] p-6 shadow-sm border border-slate-200">
                            <div class="flex justify-between items-start mb-4">
                                ...
                                @if (auth()->id() === $item->user_id || auth()->id() === $kelas->teacher_id)
                                    <div class="relative" onclick="event.stopPropagation()">
                                        <button onclick="toggleDropdown('menu-pengumuman-{{ $item->id }}')"
                                            class="p-1.5 text-slate-400 hover:text-[#092C4C] hover:bg-slate-50 rounded-md transition-colors"><svg
                                        ...
                                        <div id="menu-pengumuman-{{ $item->id }}"
                                            class="hidden absolute right-0 mt-1 w-36 bg-white rounded-lg shadow-lg border border-slate-100 z-50 overflow-hidden">
                                            @if (auth()->id() === $item->user_id)
                                                <button type="button"
                                                    onclick="openEditAnnouncement({{ $item->id }})"
                                                    class="block w-full text-left px-4 py-2.5 text-xs font-medium text-slate-600 hover:bg-slate-50 hover:text-[#00A79D]">
                                                    Edit
                                                </button>
                                            @endif
                                            <form action="{{ route('teacher.announcements.destroy', $item->id) }}"
                                                method="POST" class="delete-form">@csrf @method('DELETE')<button
                                                    type="button"
                                                    class="btn-delete block w-full text-left px-4 py-2.5 text-xs font-medium text-red-500 hover:bg-red-50 transition-colors border-t border-slate-50">Hapus</button>
                                            </form>
                                        </div>
                                    </div>
                            ...
                            @if (is_array($item->files) && count($item->files) > 0)
                                <div class="border-t border-slate-100 pt-4 mt-4">
                                    <p class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">File
                                        Terlampir</p>
                                            ...
                                            @if (is_array($file) && isset($file['path']))
                                                <a href="{{ asset('storage/' . $file['path']) }}" target="_blank"
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        <p class="text-xs font-semibold text-slate-700 truncate group-hover/file:text-[#00A79D]"
                    ...
                    @else
                        <a href="{{ route('teacher.meetings.show', $item->id) }}"
                            class="block bg-white rounded-[15px] shadow-sm border border-slate-200 hover:border-[#00A79D] hover:shadow-md transition-all group relative overflow-hidden">
                            <div
                                class="absolute left-0 top-0 bottom-0 w-1 {{ $item->type == 'tugas' ? 'bg-orange-400' : 'bg-blue-400' }}">
                            </div>
                            <div class="p-5 flex items-start gap-4">
                                <div
                                    class="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300 {{ $item->type == 'tugas' ? 'bg-orange-50 text-orange-500' : 'bg-blue-50 text-blue-500' }}">
                                    @if ($item->type == 'tugas')
                                ...
                                <div class="flex-1 pt-1">
                                    <h3
                                        class="font-bold text-[#092C4C] text-base group-hover:text-[#00A79D] transition-colors leading-tight mb-1">
                                        {{ $kelas->teacher->name }} memposting {{ $item->type }} baru:
                                        {{ $item->title }}</h3>
                                    <span
                            ...
                            @foreach ($upcomingTasks as $task)
                                <div class="group">
                                    <a href="{{ route('teacher.meetings.show', $task->id) }}" class="block">
                                        <p
                                            class="text-sm font-semibold text-slate-700 group-hover:text-[#00A79D] transition-colors line-clamp-1">
                                            {{ $task->title }}</p>
        ...
        <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center mb-8 gap-4">
                ...
                <form id="searchMeetingForm" class="flex flex-col sm:flex-row gap-3 items-center">
                    ...
                    <div class="w-full sm:w-40 flex-shrink-0">
                        <select name="type_filter"
                            ...
                            <option value="materi" {{ request('type_filter') == 'materi' ? 'selected' : '' }}>Materi
                            </option>
                            <option value="tugas" {{ request('type_filter') == 'tugas' ? 'selected' : '' }}>Tugas
                            </option>
                        </select>
                    </div>
                    <div class="w-full sm:w-40 flex-shrink-0">
            ...
            <div class="relative w-full xl:w-auto flex-shrink-0">
                <button onclick="toggleDropdown('dropdownBuatPertemuan')"
                    class="w-full xl:w-auto bg-[#00A79D] hover:bg-[#008f87] text-white px-5 py-[9px] rounded-[10px] text-sm font-medium flex items-center justify-center gap-2 transition-all shadow-sm shadow-teal-100">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                        stroke-width="2">
                ...
                <div id="dropdownBuatPertemuan"
                    class="hidden absolute right-0 mt-2 w-full xl:w-48 bg-white rounded-lg shadow-xl border border-slate-100 z-50 overflow-hidden">
                    <a href="{{ route('teacher.meetings.create', ['kelas_id' => $kelas->id, 'type' => 'tugas']) }}"
                        class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#00A79D] flex items-center gap-2"><svg
                            class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-width="2"
                                d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01">
                            </path>
                        </svg> Buat Tugas</a>
                    <a href="{{ route('teacher.meetings.create', ['kelas_id' => $kelas->id, 'type' => 'materi']) }}"
                        class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-[#00A79D] flex items-center gap-2 border-t border-slate-50"><svg
                            ...
                            <div
                                ...
                                <div class="p-4 flex justify-between items-start cursor-pointer rounded-[10px]"
                                    ...
                                    <div class="flex items-center gap-2 pl-2">
                                        <div class="relative z-10" onclick="event.stopPropagation()">
                                            <button onclick="toggleDropdown('menu-{{ $meeting->id }}')"
                                                class="p-1.5 text-slate-400 hover:text-[#092C4C] hover:bg-slate-50 rounded-md transition-colors"><svg
                                                    width="18" height="18" viewBox="0 0 24 24" fill="none"
                                                    stroke="currentColor" stroke-width="2">
                                                    <circle cx="12" cy="12" r="1" />
                                            ...
                                            <div id="menu-{{ $meeting->id }}"
                                                class="hidden absolute right-0 mt-1 w-36 bg-white rounded-lg shadow-lg border border-slate-100 z-50 overflow-hidden">
                                                <a href="{{ route('teacher.meetings.edit', $meeting->id) }}"
                                                    class="block px-4 py-2.5 text-xs font-medium text-slate-600 hover:bg-slate-50 hover:text-[#00A79D]">Edit</a>
                                                <form action="{{ route('teacher.meetings.destroy', $meeting->id) }}"
                                                    method="POST" class="delete-form">@csrf @method('DELETE')<button
                                                        type="button"
                                                        class="btn-delete block w-full text-left px-4 py-2.5 text-xs font-medium text-red-500 hover:bg-red-50 transition-colors border-t border-slate-50">Hapus</button>
                                                </form>
                                ...
                                <div id="accordion-{{ $meeting->id }}"
                                    ...
                                    <div class="p-5 md:p-6 text-sm text-slate-600 border-t border-slate-100">
                                        ...
                                        @endif
                                        <div class="mt-4 pt-4 border-t border-slate-200"><a
                                                href="{{ route('teacher.meetings.show', $meeting->id) }}"
                                                class="inline-flex items-center text-sm font-semibold text-[#00A79D] hover:text-[#008f87] hover:underline transition-colors">Lihat
                                                Selengkapnya<svg class="w-4 h-4 ml-1" fill="none"
        ...
        <div class="bg-white rounded-[20px] shadow-sm border border-slate-200 p-6">
            <div class="flex justify-between items-center mt-1 mb-6 border-b border-slate-100 pb-4">
                ...
                <form id="searchAnggotaForm" class="w-full md:w-auto">
                    <div class="w-full md:w-auto"><x-ui.search-bar placeholder="Cari nama atau email siswa..."
                            target="anggotaResultContainer" formId="searchAnggotaForm" /></div>
                                ...
                                <tr class="border-t border-slate-200 hover:bg-slate-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex justify-center">
                                            <form
                                                action="{{ route('teacher.kelas.removeStudent', ['kelas' => $kelas->id, 'student' => $student->id]) }}"
                                                method="POST" class="remove-student-form">@csrf @method('DELETE')<button
                                                    type="button"
                                                    class="btn-remove-student text-red-500 hover:text-white font-medium text-xs bg-red-50 hover:bg-red-500 px-4 py-2 rounded-lg transition-colors flex items-center gap-1"><svg
                                                        class="w-3.5 h-3.5" fill="none" stroke="currentColor"
                                                        viewBox="0 0 24 24">
                                                        <path stroke-width="2"
                                                            d="M13 7a4 4 0 11-8 0 4 4 0 018 0zM9 14a6 6 0 00-6 6v1h12v-1a6 6 0 00-6-6zM21 12h-6">
                                                        </path>
                                                    </svg> Keluarkan</button></form>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-10 text-slate-400">
                                        <div class="flex flex-col items-center gap-2"><svg class="w-10 h-10 opacity-30"
                                                fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-width="2"
                                                    d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z">
                                                </path>
                                            </svg><span class="font-medium mt-2">Belum ada siswa yang bergabung</span>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
    ...
    <div id="modalPengumuman" class="hidden fixed inset-0 z-[60] overflow-y-auto">
        <div class="fixed inset-0 bg-gray-900 bg-opacity-50 transition-opacity backdrop-blur-sm"
            onclick="toggleModal('modalPengumuman')"></div>
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div
                ...
                <div class="bg-white px-6 pb-4 pt-5 border-b border-slate-100 flex justify-between items-center">
                    ...
                    <button type="button" onclick="toggleModal('modalPengumuman')"
                        class="text-gray-400 hover:text-red-500 transition-colors p-1 rounded-md hover:bg-red-50"><svg
                            class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                ...
                <form action="{{ route('teacher.announcements.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="px-6 py-5 space-y-6">
                        <div>
                            <label class="block text-sm font-semibold text-[#092C4C] mb-2">Isi Pengumuman <span
                                    class="text-red-500">*</span></label>
                            <div
                                class="border border-slate-300 rounded-[8px] overflow-hidden focus-within:border-[#00A79D] transition-all">
                                <div id="announcement-editor-container" class="bg-white min-h-[120px] text-sm"></div>
                            </div>
                        ...
                        <div>
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <button type="button" onclick="addModalLinkField()"
                                    class="text-[#00A79D] text-xs font-semibold flex items-center gap-1 hover:bg-teal-50 px-2 py-1 rounded-md transition-colors"><svg
                                        width="12" height="12" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2">
                                        <line x1="12" y1="5" x2="12" y2="19"></line>
                                        <line x1="5" y1="12" x2="19" y2="12"></line>
                                    </svg> Tambah Link</button>
                            </div>
                            <div id="modal-link-container" class="space-y-3">
                                <div class="flex gap-2"><input type="url" name="links[]"
                        ...
                        <div class="pt-4 border-t border-slate-100">
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <button type="button" onclick="addModalFileField()"
                                    class="text-[#00A79D] text-xs font-semibold flex items-center gap-1 hover:bg-teal-50 px-2 py-1 rounded-md transition-colors"><svg
                                        width="12" height="12" viewBox="0 0 24 24" fill="none"
                                        stroke="currentColor" stroke-width="2">
                                        <line x1="12" y1="5" x2="12" y2="19"></line>
                                        <line x1="5" y1="12" x2="19" y2="12"></line>
                                    </svg> Tambah File</button>
                            </div>
                            <div id="modal-file-container" class="space-y-3">
                                <div class="flex gap-2 items-center"><input type="file" name="files[]"
                    ...
                    <div
                        class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 rounded-b-[15px] border-t border-slate-100">
                        <button type="submit"
                            class="bg-[#00A79D] hover:bg-[#008f87] text-white px-5 py-2 rounded-[8px] text-sm font-medium transition-all shadow-md shadow-teal-100">Posting
                            Pengumuman</button>
                        <button type="button" onclick="toggleModal('modalPengumuman')"
                            class="bg-white border border-slate-300 text-slate-600 hover:bg-slate-50 px-5 py-2 rounded-[8px] text-sm font-medium transition-all">Batal</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div id="modalEditPengumuman" class="hidden fixed inset-0 z-[60] overflow-y-auto">
        <div class="fixed inset-0 bg-gray-900 bg-opacity-50 transition-opacity backdrop-blur-sm"
            onclick="toggleModal('modalEditPengumuman')"></div>
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div
                class="relative transform overflow-hidden rounded-[15px] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl">
                <div class="bg-white px-6 pb-4 pt-5 border-b border-slate-100 flex justify-between items-center">
                    <h3 class="text-lg font-bold text-[#092C4C]">Edit Pengumuman</h3>
                    <button type="button" onclick="toggleModal('modalEditPengumuman')"
                        class="text-gray-400 hover:text-red-500 transition-colors p-1 rounded-md hover:bg-red-50"><svg
                            class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                            <path d="M6 18L18 6M6 6l12 12" />
                        </svg></button>
                ...
                <form id="editAnnouncementForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="px-6 py-5 space-y-6">
                        <div>
                            ...
                            <div
                                class="border border-slate-300 rounded-[8px] overflow-hidden focus-within:border-[#00A79D] transition-all">
                                <div id="edit-announcement-editor-container" class="bg-white min-h-[120px] text-sm"></div>
                            </div>
                            <input type="hidden" name="content" id="edit-announcement-content">
                        </div>
                        <div class="bg-orange-50 text-orange-600 p-3 rounded-lg text-xs flex gap-2 items-start"><svg
                                class="w-4 h-4 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor" stroke-width="2">
                                <path d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                            <p>Fitur edit saat ini hanya untuk mengubah <strong>Teks Pengumuman</strong>. Jika ingin
                                mengubah file atau link lampiran, silakan hapus pengumuman ini dan buat yang baru.</p>
                        </div>
                    </div>
                    <div
                        class="bg-slate-50 px-6 py-4 flex flex-row-reverse gap-3 rounded-b-[15px] border-t border-slate-100">
                        <button type="submit"
                            class="bg-[#00A79D] hover:bg-[#008f87] text-white px-5 py-2 rounded-[8px] text-sm font-medium transition-all shadow-md shadow-teal-100">Simpan
                            Perubahan</button>
                        <button type="button" onclick="toggleModal('modalEditPengumuman')"
                            class="bg-white border border-slate-300 text-slate-600 hover:bg-slate-50 px-5 py-2 rounded-[8px] text-sm font-medium transition-all">Batal</button>
                    </div>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            window.addEventListener('click', (e) => {
                if (!e.target.closest('button') && !e.target.closest('.relative')) {
                    document.querySelectorAll('[id^="dropdown"], [id^="menu-"]').forEach(el => el.classList
                        .add('hidden'));
                }
            });
            ...
            @if ($errors->any())
                ...
                Swal.fire({
                    ...
                    text: 'Silakan periksa kembali isian formulir.',
                    confirmButtonColor: '#ef4444'
                });
            @endif

            document.querySelectorAll('.btn-delete').forEach(btn => btn.addEventListener('click', (e) =>
                confirmAction(e, 'Hapus Data?', 'Data yang dihapus tidak dapat dikembalikan!')));
            document.querySelectorAll('.btn-remove-student').forEach(btn => btn.addEventListener('click', (e) =>
                confirmAction(e, 'Keluarkan Siswa?',
                    'Siswa ini tidak akan bisa mengakses materi dan tugas di kelas ini lagi.')));
        });
    </script>
@endsection
