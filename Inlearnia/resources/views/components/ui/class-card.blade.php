...
<div
    class="bg-white rounded-[15px] shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-300 relative flex flex-col overflow-hidden">
    ...
    <div class="px-[15px] pb-[15px]">
        ...
        <div class="flex justify-between items-center relative z-10">
            <div class="relative dropdown-container">
                ...
                <div
                    class="dropdown-menu hidden absolute bottom-full left-0 mb-2 w-[150px] bg-white border border-gray-100 rounded-[8px] shadow-xl overflow-hidden animate-fade-in origin-bottom-left z-50">
                    ...
                    <form action="{{ route('admin.kelas.destroy', $class->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Yakin hapus kelas?')"
            ...
            <a href="{{ route('admin.kelas.show', $class->id) }}"
                class="w-[120px] h-[35px] flex items-center justify-center rounded-[8px] bg-[#00A79D] text-white text-sm font-medium hover:bg-[#008f87] transition-all">
                Detail
            </a>
        </div>
    </div>
